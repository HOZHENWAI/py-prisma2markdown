...
class MermaidBuilder:
    ...
    def parse_relation(self, tokens:List[str], other_model_name:str):
        ...
        self.create_or_update_relation(self.current_model.model_name,
                                       other_model_name,
                                       relation_right
                                       )

    def create_or_update_relation(self,
                                  left_entity:str,
                                  right_entity:str,
                                  relation_right:str,
                                  ):
        ...
        if right_entity+left_entity in self.relation_dict.keys():
            self.relation_dict[right_entity+left_entity].update_cardinality("LEFT", relation_right)
    ...
    def update_relations(self):
        ...
        for relation in self.relation_dict.values():
            relation.existence = RelationIdentification.NOT_FORCED.value \
                if ("o" not in relation.cardinality_right)\
                or ("o" not in relation.cardinality_left)\
    ...
    def parse_lines(self, lines:List[str]) -> str:
        ...
        return "\n\n".join([model.to_mermaid() for model in self.model_list]
                         +[relation.to_mermaid()
                           for relation in self.relation_dict.values()])
